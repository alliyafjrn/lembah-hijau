import UserNavbar from "@/components/UserNavbar";
import Footer from "@/components/Footer";

export default function TentangPage() {
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col text-black">
      <UserNavbar />
      
      {/* Container utama dengan padding responsif */}
      <div className="flex-grow max-w-5xl mx-auto bg-white p-4 md:p-8 rounded-lg shadow-md mt-6 w-full md:w-11/12">
        
        <h1 className="text-2xl md:text-4xl font-bold text-green-700 mb-6">
          Tentang Lembah Hijau
        </h1>
        
        <p className="mb-4 text-sm md:text-base leading-relaxed">
          Lembah Hijau merupakan salah satu destinasi wisata keluarga yang berada di Bandar Lampung.
        </p>
        
        <h2 className="text-xl md:text-2xl font-semibold mt-8 mb-3">Visi</h2>
        <p className="text-sm md:text-base leading-relaxed">
          Menjadi destinasi wisata keluarga terbaik di Provinsi Lampung.
        </p>
        
        <h2 className="text-xl md:text-2xl font-semibold mt-8 mb-3">Misi</h2>
        <ul className="list-disc ml-5 space-y-2 text-sm md:text-base leading-relaxed">
          <li>Menyediakan wisata edukatif.</li>
          <li>Menjaga kelestarian lingkungan.</li>
          <li>Memberikan pengalaman wisata terbaik bagi pengunjung.</li>
        </ul>
      </div>
      
      <Footer />
    </div>
  );
}